...
function logOutput(recompilableContracts: CheckedContract[], programOptions: any) {
    switch (recompilableContracts.length) {
        case 0: {
            console.log(`Could not find the ${JSON_OPTION} target: ${programOptions.prepareJson}`);
            process.exitCode = 1;
            break;
        }
        case 1: {
        ...
        default: {
            console.log(`Ambiguous ${JSON_OPTION} target: ${programOptions.prepareJson}`);
...
const JSON_OPTION = "prepare-json";
...
program
    ...
    .description(
        ...
        "Missing sources are fetched from IPFS, if URLs are provided.")
    .usage("[options] <path>...")
    .option(
        `-j, --${JSON_OPTION} <path:name>`,
        ...
        `E.g. \`${name} --${JSON_OPTION} foo/bar/Contract.sol:ContractName path/to/dir path/to/zip\`\n`
    )
    .option(
        "-S, --no-settings",
        "Omit the original settings when preparing a standard-json.\n" +
        `Only available with the --${JSON_OPTION} option\n`,
        true
    )
    .option(
        "-p, --pretty",
    ...
    checkedContracts.forEach(contract => {
        if (matches(contract, recompilationTarget)) {
            if (contract.isValid()) {
                ...
                console.log(`The ${JSON_OPTION} target is not a valid contract! Exiting.`);
                process.exit(1);
            }
            ...
            if (contract.isValid()) {
                msg += "\n";
                msg += `  To recompile, use: \`${name} --${JSON_OPTION} ${contract.compiledPath}:${contract.name} ${fileNamesJoint}`;
